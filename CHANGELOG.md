...
之前 LLM 靠 tool 的 `description` 字段猜何时调哪个 tool，经常**瞎调或漏调**。
v0.2.0 后每个 tool 都有：

- `promptSnippet` — 一句话用途（"用 X 来做 Y"）
- `promptGuidelines` — 数组形式的调用规则（"调 X 前先调 Y"、"参数 Z 必填"）

**效果**：跟 pi 说"找包"时，**调得更准、少走弯路、回答更靠谱**。

**举例**：
- 之前：用户问"pi-autoname 安全吗？"→ LLM 可能直接编一个"安全"的答案
- 之后：LLM 看 `marketplace_audit` 的 `promptGuidelines` 知道"用户问安全时必须先调这个 tool 拿真实审计数据"
...
### 改进明细（5 项，对应借鉴路线图 § 2.1）

- **#1** `promptSnippet` + `promptGuidelines` 补齐 4 个 tool
- **#2** `security.ts` 三项同步：`maxBuffer` 32MB / `AbortSignal` / HTTP URL 白名单
- **#3** 安装进度 `setStatus` spinner
- **#4** `tools/` 目录重组（4 并列 → 合并/分组）
- **#5** i18n 基础设施：`extensions/i18n.ts`，`t()` 函数，硬编码 zh-CN + en
...
- 借鉴路线图：`从-pi-packages-manager-借鉴-架构与UX路线图.md` § 2.1 #1-#5
